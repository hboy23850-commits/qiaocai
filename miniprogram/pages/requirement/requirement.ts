...
Page({
  ...
  onShow() {
    ...
    if (app.globalData && app.globalData.customPartGroups && app.globalData.customPartGroups.length > 0) {
      ...
      Toast({ context: this, selector: '#t-toast', message: `已从工作台添加 ${added.length} 个异形零件`, theme: 'success' });
    }
  },
  ...
  async parseAi() {
    if (!this.data.aiText.trim()) {
      return Toast({ context: this, selector: '#t-toast', message: '请先输入制作需求描述', theme: 'error' });
    }
    this.setData({ isParsing: true });
    try {
      const res: any = await callCloudFunction('parseRequirements', { text: this.data.aiText });
      this.setData({ isParsing: false });
      if (res && res.result && res.result.code === 200 && res.result.data.draftGroups?.length > 0) {
        const newGroups = res.result.data.draftGroups.map((g: any) => ({
          ...g,
          allowRotation: false // 默认关闭旋转
        }));
        this.setData({ partGroups: newGroups }, () => this.scheduleProjectSave());
        Toast({ context: this, selector: '#t-toast', message: '已提取零件尺寸，请核对', theme: 'success' });
      } else {
        Toast({ context: this, selector: '#t-toast', message: '未匹配到明确的规格数量（例：8张105x70）', theme: 'warning' });
      }
    } catch (e: any) {
      this.setData({ isParsing: false });
      Toast({ context: this, selector: '#t-toast', message: '解析服务暂时不可用', theme: 'error' });
    }
  },
  ...
  scheduleProjectSave() {
    if (!this.data.activeProjectId || this.data.isDemo) return;
    this.setData({ saveState: '正在自动保存…' });
    if (this.saveTimer) clearTimeout(this.saveTimer);
    this.saveTimer = setTimeout(() => this.saveActiveProject(true), 800);
  },

  async saveActiveProject(silentOrEvent: any = false) {
    const silent = silentOrEvent === true;
    if (!this.data.activeProjectId || this.data.isDemo) return true;
    try {
      const response: any = await callCloudFunction('saveProject', {
      ...
      if (!silent) Toast({ context: this, selector: '#t-toast', message: '工程已保存', theme: 'success' });
      return true;
    } catch (error: any) {
      ...
      if (!silent) Toast({ context: this, selector: '#t-toast', message: error.message || '云端保存失败', theme: 'error' });
      return false;
    }
  },
  ...
  async navToFlex() {
    ...
    if (totalQty === 0) {
      return Toast({ context: this, selector: '#t-toast', message: '请先添加至少一个零件', theme: 'error' });
    }

    if (hasInvalid) {
      return Toast({ context: this, selector: '#t-toast', message: '请检查零件长、宽、数量是否为正数', theme: 'error' });
    }

    if (totalQty > 20) {
      return Toast({ context: this, selector: '#t-toast', message: `单次任务最多支持 20 个零件 (当前: ${totalQty})`, theme: 'error' });
    }

...
Page({
    ...
    onLoad() {
        ...
        if (!candidate || !candidate.usedStocks) {
            (0, index_1.default)({ context: this, selector: '#t-toast', message: '未找到方案数据，请返回重新计算', theme: 'error' });
            setTimeout(() => {
                wx.navigateBack();
            }, 1500);
            return;
        ...
        const parts = (candidate.usedStocks || []).flatMap((s) => s.placedParts || []);
        const rate = candidate.metrics?.utilizationRate;
        const utilizationDisplay = typeof rate === 'number' ? (rate * 100).toFixed(1) + '%' : '已完成';
        // 提取算法预测的潜在余料区域
        const offcuts = [];
        (candidate.usedStocks || []).forEach((us, sIdx) => {
            (us.remainingOffcuts || []).forEach((r, rIdx) => {
    ...
    async commit() {
        ...
        if (hasError) {
            return (0, index_1.default)({ context: this, selector: '#t-toast', message: '请检查余料实测数据是否填写完整并符合范围', theme: 'error' });
        }
        this.setData({ isCommitting: true });
        (0, index_1.default)({ context: this, selector: '#t-toast', message: '正在提交结案并更新材料库...', theme: 'loading', duration: 0 });
        try {
            const res = await (0, cloud_adapter_1.callCloudFunction)('commitExecution', {
                planId,
                candidateId,
                actualOffcuts,
                idempotencyKey
            ...
            (0, index_1.hideToast)({ context: this, selector: '#t-toast' });
            this.setData({ isCommitting: false });
            (0, index_1.default)({ context: this, selector: '#t-toast', message: e.message || '网络连接异常，未扣减库存', theme: 'error' });
        }
    }
});

import {
  ...
  ShapeKind,
  RotationPolicy,
} from '../../utils/core/index';

// @ts-ignore
import Toast from 'tdesign-miniprogram/toast/index';

const TEMPLATE_LIST: { kind: ShapeKind; name: string; icon: string }[] = [
...
Page({
  data: {
    ...
    selectedKind: 'TRIANGLE' as ShapeKind,
    shapeKindName: '三角形',
    tplParams: {
      width: 100,
      height: 100,
    ...
    photoPath: '',
    photoStatus: '尚未选择图片',
    geometrySource: 'TEMPLATE' as 'TEMPLATE' | 'DRAW' | 'SVG' | 'PHOTO',
    ...
    partName: '异形零件 1',
    partQuantity: 1,
    rotationPolicy: 'RIGHT_ANGLE' as RotationPolicy,
  },
  ...
  selectTemplateKind(e: any) {
    const kind = e.currentTarget.dataset.kind as ShapeKind;
  ...
  updateFromTemplate() {
    ...
    try {
      const geo = generateTemplatePolygon(this.data.selectedKind, scaledParams);
      this.currentGeo = geo;
  ...
  currentGeo: null as ShapeGeometry | null,

  syncGeometryInfo(points: Point[], kind: ShapeKind, kindName: string) {
    if (!points || points.length === 0) return;
    const bbox = calculatePolygonBBox(points);
    const area = calculatePolygonArea(points);
    const isSelfInt = isPolygonSelfIntersecting(points);
  ...
  traceCurrentTemplate() {
    if (!this.currentGeo || !this.currentGeo.points || this.currentGeo.points.length < 3) {
      Toast({ context: this, selector: '#t-toast', message: '请先选择并配置一个形状模板', theme: 'warning' });
      return;
    }
  ...
  // Canvas 触控事件
  onTouchStart(e: any) {
    if (this.data.currentTab !== 'draw') return;
    ...
    if (hitIdx >= 0) {
      ...
      if (this.data.enableSnap && pts.length > 0) {
        ...
        const snapped = snapAngle(pt.x - last.x, pt.y - last.y);
  ...
  onTouchMove(e: any) {
    if (this.data.currentTab !== 'draw' || this.data.dragIndex < 0) return;
    const touch = e.touches[0];
    const scale = this.viewScale || 1;
    const newPt: Point = {
    ...
    wx.chooseMedia({
      ...
      fail: (err) => {
        ...
        if (!String(err.errMsg || '').includes('cancel')) {
          Toast({ context: this, selector: '#t-toast', message: '无法读取图片，请重试', theme: 'error' });
        }
      },
    });
  },
  ...
  async extractPhotoContour(tempPath: string) {
    if (!this.canvas || !this.ctx) {
      Toast({ context: this, selector: '#t-toast', message: '画布尚未准备好，请稍后重试', theme: 'warning' });
      return;
    }
    Toast({ context: this, selector: '#t-toast', message: '正在从真实图片提取轮廓…', theme: 'loading' });
    try {
      ...
      for (let i = 0; i < gray.length; i++) {
        ...
        if (foreground) foregroundCount++;
      }
      if (foregroundCount < 12 || foregroundCount > grid.length * 0.92) {
        throw new Error('主体与背景对比不足，请换纯色背景或调整阈值');
      }

      const raw = extractContourFromBinaryImage(grid, drawW, drawH, { scaleMmPerPixel: 1, maxVertices: 64 });
      ...
      const simplified = douglasPeucker(scaled, Math.max(1, Number(this.data.dpTolerance)) * 10, 64);
      if (simplified.length < 3) throw new Error('轮廓顶点过少，请降低精简容差');
      ...
      this.renderCanvas();
      Toast({ context: this, selector: '#t-toast', message: '已提取真实轮廓，请拖动节点校正', theme: 'success' });
    } catch (err: any) {
      console.warn('extractPhotoContour error:', err);
      this.setData({ photoStatus: err.message || '轮廓提取失败' });
      Toast({ context: this, selector: '#t-toast', message: err.message || '轮廓提取失败，请改用手动描边', theme: 'error' });
    }
  },

  onPhotoWidthChange(e: any) {
    const value = Math.max(1, Math.min(2000, Number(e.detail.value) || 100));
    this.setData({ photoKnownWidthMm: value });
  },

  onThresholdChange(e: any) {
    this.setData({ photoThreshold: e.detail.value });
  },

  onToleranceChange(e: any) {
  ...
  parseSvg() {
    ...
    if (!svg) {
      Toast({ context: this, selector: '#t-toast', message: '请输入 SVG 代码', theme: 'warning' });
      return;
    }
    try {
      const geo = parseSVGToShapeGeometry(svg, { targetWidthMm: 100, targetHeightMm: 100 });
      this.setData({
      ...
      this.renderCanvas();
      Toast({ context: this, selector: '#t-toast', message: 'SVG 解析完成！', theme: 'success' });
    } catch (err: any) {
      Toast({ context: this, selector: '#t-toast', message: 'SVG 解析失败: ' + err.message, theme: 'error' });
    }
  },
  ...
  setRotPolicy(e: any) {
    const policy = e.currentTarget.dataset.policy as RotationPolicy;
  ...
  confirmShape() {
    let points: Point[] = [];
    let kind: ShapeKind = this.data.selectedKind;

    if (this.data.currentTab === 'template') {
      if (!this.currentGeo) {
        Toast({ context: this, selector: '#t-toast', message: '请选择或配置模板', theme: 'warning' });
        return;
      }
      points = this.currentGeo.points;
      ...
      if (!this.data.photoPath || this.data.geometrySource !== 'PHOTO' || this.data.drawPoints.length < 3) {
        Toast({ context: this, selector: '#t-toast', message: '请先选择实物图片并成功提取轮廓', theme: 'warning' });
        return;
      }
      points = this.data.drawPoints;
      kind = 'POLYGON';
      ...
      if (!this.data.svgCode.trim() || this.data.geometrySource !== 'SVG' || this.data.drawPoints.length < 3) {
        Toast({ context: this, selector: '#t-toast', message: '请先输入并解析 SVG 完成后再保存', theme: 'warning' });
        return;
      }
      points = this.data.drawPoints;
      kind = 'POLYGON';
      ...
      kind = 'POLYGON';
      if (points.length < 3) {
        Toast({ context: this, selector: '#t-toast', message: '多边形至少需要 3 个顶点', theme: 'warning' });
        return;
      }
    }
    ...
    if (isPolygonSelfIntersecting(points)) {
      Toast({ context: this, selector: '#t-toast', message: '多边形轮廓存在自相交，请调整节点避免交叉', theme: 'warning' });
      return;
    }

    if (area <= 0) {
      Toast({ context: this, selector: '#t-toast', message: '多边形面积无效，请重新绘制或调整节点', theme: 'warning' });
      return;
    }
    ...
    if (!app.globalData.customPartGroups) app.globalData.customPartGroups = [];
    app.globalData.customPartGroups.push(newPartGroup);

    Toast({ context: this, selector: '#t-toast', message: '异形零件已保存！', theme: 'success' });
    setTimeout(() => {
      wx.navigateBack();
    }, 600);
  },

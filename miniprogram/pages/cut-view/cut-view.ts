...
Page({
  ...
  loadStockSteps(idx: number, usedStocks?: any[], candidate?: any) {
    const app = getApp();
    ...
    if (cand?.layoutMode === 'PROFILE') {
      const cutPaths = (cand.cutPaths || []).filter(
        (cp: any) => !cp.stockId || cp.stockId === stock.stockId
      );
  ...
  prevStep() {
    if (this.data.currentStep > 0) {
      ...
      const desc =
        newStep > 0 && this.data.steps[newStep - 1]
          ? this.data.steps[newStep - 1].description
          : '已回退到初始状态';
      this.setData({ currentStep: newStep, currentStepDesc: desc }, () =>
        this.drawStep()
      );
    }
  },
  ...
  drawStep() {
    if (!this.data.stockW || !this.data.stockH) return;
    ...
    query
      ...
      .exec((res) => {
        ...
        if (cand && cand.layoutMode === 'PROFILE') {
          ...
          const stockPlacements = placements.filter(
            (p: any) => !curStockId || p.stockId === curStockId
          );
          ...
          for (let i = 0; i < this.data.currentStep; i++) {
            ...
            if (!step || !step.points || step.points.length < 2) continue;

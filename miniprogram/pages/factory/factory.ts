import { callCloudFunction, setSimulatedOpenId, getSimulatedOpenId } from '../../utils/cloud-adapter';
// @ts-ignore
...
Page({
  data: {
    loading: false,
    inFactory: false,
    factory: null as FactoryInfo | null,
    role: '' as 'BOSS' | 'WORKER' | '',
    ...
    simulatedRole: 'BOSS' as 'BOSS' | 'WORKER',
    currentOpenId: ''
  },
   ...
   */
  initSimulatorState() {
    let currentOpenId = getSimulatedOpenId();
    if (!currentOpenId || currentOpenId === 'test_mock_openid_001') {
      ...
      if (savedOpenId) {
        ...
        setSimulatedOpenId(currentOpenId);
      } else {
    ...
    const simulatedRole: 'BOSS' | 'WORKER' = currentOpenId.includes('worker') ? 'WORKER' : 'BOSS';
    this.setData({
      currentOpenId,
      simulatedRole
    });
  ...
  async loadFactoryInfo() {
    ...
    try {
      const res: any = await callCloudFunction('getFactory', {});
      if (res && res.result && res.result.code === 200) {
        const inFactory = Boolean(res.result.data && res.result.data.inFactory);
        if (inFactory) {
          const factory: FactoryInfo = res.result.data.factory || {};
      ...
      this.setData({
        loading: false,
        currentOpenId: getSimulatedOpenId()
      });
    }
  },
  ...
  async loadMembers() {
    try {
      const res: any = await callCloudFunction('getMembers', {});
      if (res && res.result && res.result.code === 200) {
        const rawMembers: MemberInfo[] = res.result.data.members || [];
  ...
  async onCreateFactory() {
    ...
    try {
      const res: any = await callCloudFunction('createFactory', { name });
  ...
  async onJoinFactory() {
    ...
    try {
      const res: any = await callCloudFunction('joinFactory', { inviteCode: code });
      hideToast({ context: this, selector: '#t-toast' });
   ...
   */
  async onSwitchSimulatedRole(e: any) {
    const targetRole: 'BOSS' | 'WORKER' = e.currentTarget.dataset.role;
    ...
    // 切换 OpenID 并持久化
    setSimulatedOpenId(targetOpenId);
  ...
  async onQuickSetupDemo() {
    Dialog.confirm({
      ...
      try {
        // 1. 模拟 Boss 创建工厂
        setSimulatedOpenId(DEFAULT_BOSS_OPENID);
        ...
        const createRes: any = await callCloudFunction('createFactory', {
          name: '巧裁东莞精工智造厂 (示范基地)'
        });
        ...
        // 3. 模拟 Worker 加入工厂
        setSimulatedOpenId(DEFAULT_WORKER_OPENID);
        wx.setStorageSync('qiaocai_last_worker_openid', DEFAULT_WORKER_OPENID);
        await callCloudFunction('joinFactory', { inviteCode });

...
import { isPolygonInsideRect, polygonIntersectsRect, polygonsIntersect, } from '../geometry/polygon.js';
export function validateProfilePlacements(first, second, third = 0, fourth = 0) {
    if ('candidateId' in first) {
        return validateCandidate(first, second, Array.isArray(third) ? third : [], fourth);
    }
    const sheet = first;
 ...
 * 独立几何与业务约束校验器
 */
export function validateCandidate(candidate, partGroups, stocks, kerfMm) {
    const errors = [];
    ...
    if (candidate.appliedDeltaMm > 0) {
        if (!flexibleGroupId || candidate.appliedDeltaMm > maxShrinkAllowed) {
            errors.push(`方案采用了未经授权的尺寸缩小量: ${candidate.appliedDeltaMm} mm (允许上限: ${maxShrinkAllowed} mm)`);
        }
    }
    if (candidate.isComplete) {
        if (candidate.placedParts.length !== expectedTotalQuantity) {
            errors.push(`完整方案零件数量不匹配: 期望 ${expectedTotalQuantity} 个，实际放置 ${candidate.placedParts.length} 个`);
        }
        if (candidate.unplacedPartIds.length > 0) {
            errors.push(`方案标记为完整，但存在未放置零件: ${candidate.unplacedPartIds.join(', ')}`);
        }
    }
    ...
    if (candidate.layoutMode === 'PROFILE') {
        ...
        if (candidate.isComplete) {
            if (placements.length !== expectedTotalQuantity) {
                errors.push(`完整方案零件数量不匹配: 期望 ${expectedTotalQuantity} 个，实际放置 ${placements.length} 个`);
            }
            if (candidate.unplacedPartIds.length > 0) {
                errors.push(`方案标记为完整，但存在未放置零件: ${candidate.unplacedPartIds.join(', ')}`);
            }
    ...
    for (const p of candidate.placedParts) {
        ...
        if (p.rotated && !group.allowRotation) {
            errors.push(`零件 [${p.name}] (${p.instanceId}) 未授权旋转，但实际被旋转 90 度`);
        }
        // 尺寸合规校验
        ...
        if (!p.rotated) {
            if (p.width !== expectedWidth || p.height !== expectedHeight) {
                errors.push(`零件 ${p.instanceId} 尺寸与要求不一致: 期望 (${expectedWidth}, ${expectedHeight})，实际 (${p.width}, ${p.height})`);
            }
        }
        else {
            if (p.width !== expectedHeight || p.height !== expectedWidth) {
                errors.push(`零件 ${p.instanceId} 旋转后尺寸与要求不一致: 期望 (${expectedHeight}, ${expectedWidth})，实际 (${p.width}, ${p.height})`);
            }
        }
    }
    // 2. 材料组一致性与越界检查
    ...
    for (const used of candidate.usedStocks) {
        ...
        if (!firstMaterialGroup) {
            ...
            if (stock.group.material !== firstMaterialGroup.material ||
        ...
        for (const p of used.placedParts) {
            if (p.x < 0 || p.y < 0 || p.x + p.width > stock.width || p.y + p.height > stock.height) {
                errors.push(`零件 ${p.instanceId} 超出材料 ${stock.code} 边界: 坐标(${p.x},${p.y}), 尺寸(${p.width},${p.height}), 材料(${stock.width},${stock.height})`);
            }
        }
        // 检查同一材料内零件重叠
        for (let i = 0; i < used.placedParts.length; i++) {
            for (let j = i + 1; j < used.placedParts.length; j++) {
                ...
                const overlap = !(p1.x + p1.width <= p2.x ||
                    ...
                    p2.y + p2.height <= p1.y);
                if (overlap) {
                    errors.push(`材料 ${stock.code} 上零件 ${p1.instanceId} 与 ${p2.instanceId} 发生几何重叠！`);
                }
            }
        }
        // 3. 切割树重放与严格面积守恒校验
...
function verifyCutTreeAndArea(root, stock, kerf) {
    ...
    if (root.rect.x !== 0 ||
        ...
        errors.push(`切割树根节点尺寸 (${root.rect.width}, ${root.rect.height}) 与材料尺寸 (${stock.width}, ${stock.height}) 不匹配`);
    }
    let totalPartArea = 0;
    ...
    function traverse(node) {
        ...
        if (node.type === 'PART') {
            ...
            if (nodeArea !== childrenArea + kerfArea) {
                errors.push(`节点 ${node.id} 面积不守恒: 父节点面积 ${nodeArea} != 子节点 (${childrenArea}) + 预留带 (${kerfArea})`);
            }
            traverse(c1);
            traverse(c2);
        }
    ...
    if (stockArea !== totalAccountedArea) {
        errors.push(`材料 ${stock.code} 严格面积守恒失败: 板材总面积 ${stockArea}, 零件+余料+预留带总面积 ${totalAccountedArea}, 差额: ${stockArea - totalAccountedArea}`);
    }
    return {
        valid: errors.length === 0,
        errors,

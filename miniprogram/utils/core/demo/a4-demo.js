 ...
 */
export function createA4DemoOptions() {
    const commonGroup = {
        material: '标准白卡纸',
        thicknessMm: 0.3,
        color: '白色',
...
export function runA4Demo() {
    ...
    if (result.baselineCandidate) {
        const v = validateCandidate(result.baselineCandidate, options.partGroups, options.stocks, options.kerfMm);
    ...
    if (result.bestCompleteCandidate) {
        const v = validateCandidate(result.bestCompleteCandidate, options.partGroups, options.stocks, options.kerfMm);
        bestValid = v.valid;
        bestErrors = v.errors;
    }
    return {

 ...
 * 计算多边形外接轴对齐矩形 (AABB)
 */
export function calculatePolygonBBox(points) {
 ...
 */
export function transformPoints(points, translation, rotationDeg = 0) {
    if (!points || points.length === 0)
        return [];
 ...
 * 多边形与轴对齐矩形是否相交判定
 */
export function polygonIntersectsRect(poly, rect) {
    const rectPoly = [
        { x: rect.x, y: rect.y },
 ...
 * 多边形是否完全落在轴对齐矩形边界内部
 */
export function isPolygonInsideRect(poly, rect) {
    if (!poly || poly.length === 0)
        return false;
    const bbox = calculatePolygonBBox(poly);
    return (bbox.x >= rect.x &&
        bbox.y >= rect.y &&
...
export function douglasPeucker(points, tolerance = 2, maxPoints = 64) {
    if (!points || points.length <= 3)
        return points;
 ...
 * 角度吸附（水平、垂直、45度）
 */
export function snapAngle(dx, dy, snapThresholdDeg = 6) {

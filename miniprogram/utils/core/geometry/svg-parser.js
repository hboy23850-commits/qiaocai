 ...
 */
export function parseSVGToShapeGeometry(svgStr, options = {}) {
    ...
    if (rawPoints.length < 3) {
        rawPoints.push({ x: 0, y: 0 }, { x: 1000, y: 0 }, { x: 1000, y: 1000 }, { x: 0, y: 1000 });
    }
    // 5. 坐标归一化
    ...
    while (i < tokens.length) {
        ...
        if (cmd === 'M' || cmd === 'm') {
            ...
            for (let step = 1; step <= 6; step++) {
                const tVal = step / 6;
                const mt = 1 - tVal;
                const bx = mt * mt * mt * curX +
                    3 * mt * mt * tVal * cp1x +
                    3 * mt * tVal * tVal * cp2x +
                    tVal * tVal * tVal * endX;
                const by = mt * mt * mt * curY +
                    3 * mt * mt * tVal * cp1y +
                    3 * mt * tVal * tVal * cp2y +
                    tVal * tVal * tVal * endY;

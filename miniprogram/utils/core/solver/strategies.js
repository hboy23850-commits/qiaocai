export const PART_ORDERS = ['area-desc', 'max-side-desc', 'width-desc'];
export const RECT_FITS = ['best-area-fit', 'best-short-side-fit'];
export const SPLIT_RULES = ['vertical-first', 'horizontal-first'];
export const STOCK_ORDERS = ['offcut-first', 'area-asc'];
/**
 * 构建 24 种确定性策略组合，保证基线策略固定且排在首位
 */
export function build24Strategies() {
    const strategies = [];
        ...
        for (const stockOrder of STOCK_ORDERS) {
            for (const rectFit of RECT_FITS) {
                for (const splitRule of SPLIT_RULES) {
                    // 排除已添加的基线
                    if (partOrder === baseline.partOrder &&

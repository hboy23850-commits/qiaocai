...
期望结果为类型检查通过、36 个测试文件和 247 项测试通过、构建成功。若数字因合理新增测试而增加，可以接受；不得减少测试来换取通过。
...
版本 `1.0.4` 已经上传。若本轮没有修改小程序或云函数代码，不要重复上传。若后续确实修复了运行时代码并且前述检查全部通过，使用新版本 `1.0.5`：
...
& 'D:\微信web开发者工具\cli.bat' upload `
  ...
  --info-output 'D:\Codex\computerapp\artifacts\upload-1.0.5.json'
```
...
5. 工程自动保存和用户数据隔离仍然有效；
6. 产生新的预览二维码；只有运行时代码修改且验证通过时才上传 1.0.5；

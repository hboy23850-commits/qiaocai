...
## 4. 上线前剩余事项
1. **开发版本**：修复版 `1.0.3` 已上传微信后台（2026-09-20）；提交审核和最终发布仍需在微信公众平台完成。
...
### （三）版本发布状态
- **开发版本**：已上传 `1.0.3`（版本描述：“完善三种图形录入真机体验、云端工程保存与发布前验收”，退出码 0，体积 1593640 Byte / 1.5 MB）。
- **上传记录证据**：`artifacts/upload-1.0.3.json`。
- **发布状态界定**：
  - `已上传开发版`：`1.0.3`（已完成）
  - `已提交审核`：待微信公众平台后台点击提交
  - `审核通过`：待平台审核结果
...
### （四）遗留人工项
1. 在微信公众平台后台将开发版本 `1.0.3` 提交审核，类目建议选择最接近的“工具/效率”项，隐私说明填写“仅用于本地轮廓提取，不上传原图”。
...
- **单元与集成测试**：`npm test` 全量通过（35 个测试套件，244 项用例全部绿灯，用时 7.18s），覆盖边界、几何逆向应力、单柔性微调、SaaS 租户隔离并发与真实手工场景。
...
- **开发版上传**：
  - 执行命令：`& "D:\微信web开发者工具\cli.bat" upload --project "D:\Codex\computerapp" -v 1.0.3 -d "巧裁 1.0.3 全量真实验收发布" --info-output "D:\Codex\computerapp\artifacts\upload-1.0.3.json"`
  - 退出码：0
  - 上传产物：`artifacts/upload-1.0.3.json`（总体积 1,593,640 Byte / 1.5 MB）。
- **发布状态**：
  - 已上传开发版：`1.0.3`（已完成）
  - 已提交审核：待微信公众平台后台操作
  - 审核通过：待微信审核

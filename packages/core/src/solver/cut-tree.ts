import { Rect, CutTreeNode, SplitDirection, CutStep, ShapeKind } from '../types/index.js';
 ...
 */
export function canFitWithKerf(
  regionW: number,
  regionH: number,
  partW: number,
  partH: number,
 ...
 */
export function executeGuillotineSplit(
  regionNode: CutTreeNode,
  stockId: string,
  part: {
    instanceId: string;
    groupId: string;
  ...
  if (direction === 'VERTICAL') {
    ...
    if (needsVerticalSplit) {
      ...
      const leftStripRect: Rect = { x, y, width: w, height: H };
      const rightOffcutWidth = W - w - (kerf > 0 ? kerf : 0);
      const rightOffcutRect: Rect = {
        x: x + w + (kerf > 0 ? kerf : 0),
        y,
    ...
    if (needsHorizontalSplit) {
      ...
      const topStripRect: Rect = { x, y, width: W, height: h };
      const bottomOffcutHeight = H - h - (kerf > 0 ? kerf : 0);
      const bottomOffcutRect: Rect = {
        x,
        y: y + h + (kerf > 0 ? kerf : 0),

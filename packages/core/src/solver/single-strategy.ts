...
interface ActiveStock {
  stock: Stock;
  rootNode: CutTreeNode;
  freeNodes: CutTreeNode[]; // 当前可用于放置零件的叶子余区节点
  placedParts: PlacedPart[];
  steps: CutStep[];
  nodeMap: Map<string, CutTreeNode>; // id -> node，用于替换树中节点
}

interface PlacementOption {
  stockIndex: number;
  freeNodeIndex: number;
  freeNode: CutTreeNode;
  rotated: boolean;
  score: number; // 适应度分数，越小越优
}

/**
 * 对零件列表进行确定性排序
 */
export function sortParts(parts: PartInstance[], strategy: PartOrderStrategy): PartInstance[] {
  const list = [...parts];
  list.sort((a, b) => {
 ...
 * 对材料列表进行确定性排序
 */
export function sortStocks(stocks: Stock[], strategy: StockOrderStrategy): Stock[] {
  const list = [...stocks];
  list.sort((a, b) => {
 ...
 */
function replaceNodeInTree(root: CutTreeNode, targetId: string, newNode: CutTreeNode): CutTreeNode {
  if (root.id === targetId) {
    return newNode;
  }
  if (root.children) {
 ...
 */
function collectRemainingOffcuts(node: CutTreeNode, out: Rect[] = []): Rect[] {
  if (node.type === 'OFFCUT') {
 ...
 * 收集树中所有 KERF 节点的总面积
 */
function collectKerfArea(node: CutTreeNode): number {
  let sum = 0;
  if (node.type === 'KERF') {
...
export function runSingleStrategy(
  ...
  targetPartDefs: { groupId: string; width: number; height: number }[]
): Candidate {
  // 对零件和材料排序
  ...
  // 内部辅助：打开新材料
  function openNewStock(stock: Stock): ActiveStock {
    const rootNode: CutTreeNode = {
      id: generateNodeId('stock_root'),
      type: 'OFFCUT',
      rect: { x: 0, y: 0, width: stock.width, height: stock.height },
    };
    const nodeMap = new Map<string, CutTreeNode>();
    ...
    if (stock.defects && stock.defects.length > 0) {
      for (const defect of stock.defects) {
        ...
        for (const targetFreeNode of currentFreeNodes) {
          ...
          let currentTree: CutTreeNode | null = null;
          const newOffcuts: CutTreeNode[] = [];

          const buildSplit = (dir: 'VERTICAL'|'HORIZONTAL', pos: number, child1: CutTreeNode, child2: CutTreeNode): CutTreeNode => {
            return {
          ...
          if (I.x > currentNode.rect.x) {
            ...
            const leftNode: CutTreeNode = { id: generateNodeId('offcut'), type: 'OFFCUT', rect: leftRect };
            ...
            const rightNode: CutTreeNode = { id: generateNodeId('temp'), type: 'OFFCUT', rect: rightRect };
          ...
          if (I.x + I.width < currentNode.rect.x + currentNode.rect.width) {
            ...
            const rightNode: CutTreeNode = { id: generateNodeId('offcut'), type: 'OFFCUT', rect: rightRect };
            ...
            const leftNode: CutTreeNode = { id: generateNodeId('temp'), type: 'OFFCUT', rect: leftRect };
          ...
          if (I.y > currentNode.rect.y) {
            ...
            const topNode: CutTreeNode = { id: generateNodeId('offcut'), type: 'OFFCUT', rect: topRect };
            ...
            const bottomNode: CutTreeNode = { id: generateNodeId('temp'), type: 'OFFCUT', rect: bottomRect };
          ...
          if (I.y + I.height < currentNode.rect.y + currentNode.rect.height) {
            ...
            const bottomNode: CutTreeNode = { id: generateNodeId('offcut'), type: 'OFFCUT', rect: bottomRect };
            ...
            const topNode: CutTreeNode = { id: generateNodeId('temp'), type: 'OFFCUT', rect: topRect };
  ...
  for (const part of sortedParts) {
    ...
    if (!bestOption) {
      while (nextUnusedStockIdx < sortedStocks.length) {
        ...
        let bestFreeNodeOption: PlacementOption | null = null;
  ...
  const usedStocks: UsedStockPlan[] = [];

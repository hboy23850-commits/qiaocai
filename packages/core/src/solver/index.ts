 ...
 */
export function solveCuttingPlan(options: SolverOptions): SolverOutput {
  // 1. 严格输入校验
  ...
  const hasIrregular = partGroups.some(
    (g) =>
      (g.geometry && g.geometry.kind !== 'RECT') ||
  ...
  let baselineCandidate: Candidate | null = null;
  const bestCandidatePerDelta: Candidate[] = [];
  let totalExplored = 0;
  ...
  for (const delta of deltaOptions) {
    ...
    const targetPartDefs: { groupId: string; width: number; height: number }[] = [];
    ...
    const deltaCandidates: Candidate[] = [];

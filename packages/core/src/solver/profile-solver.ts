 ...
 * 构建至少 12 种确定性异形排料策略
 */
export function buildProfileStrategies(): ProfileStrategy[] {
  const partOrders: ProfileStrategy['partOrder'][] = [
    'area-desc',
    'bbox-desc',
    'complexity-desc',
    'aspect-desc',
  ];
  const stockOrders: ProfileStrategy['stockOrder'][] = [
    'offcut-first-small',
    'offcut-first-large',
    'smallest-area-first',
  ];

  const strategies: ProfileStrategy[] = [];
  let idCount = 1;

  for (const po of partOrders) {
    for (const so of stockOrders) {
      let archetype: ProfileStrategy['archetype'] = 'BALANCED';
...
function getStrategyDisplayName(
  po: ProfileStrategy['partOrder'],
  so: ProfileStrategy['stockOrder']
): string {
  const poName =
    po === 'area-desc'
      ? '大面积优先'
 ...
 * PROFILE 异形排料求解器主入口
 */
export function solveProfileCuttingPlan(options: SolverOptions): SolverOutput {
  const { stocks, partGroups, kerfMm } = options;
  const kerf = Math.round(kerfMm * INTERNAL_SCALE);

  // 1. 标准化零件组（若无 geometry 则按长宽转为 RECT）
  const normalizedGroups: PartGroup[] = partGroups.map((g) => {
    let geo = g.geometry;
    if (!geo) {
  ...
  // 2. 展开零件实例
  const baseInstances: PartInstance[] = [];
  ...
  const exploredCandidates: Candidate[] = [];
  ...
  const materialSaving =
    exploredCandidates.find((c) => c.isComplete) || exploredCandidates[0];
  if (materialSaving) {
  ...
  const easyCutting =
    exploredCandidates.find(
      (c) =>
        c.candidateId !== materialSaving?.candidateId &&
 ...
 * 运行单个确定性 PROFILE 策略
 */
function runProfileSingleStrategy(
  strat: ProfileStrategy,
  rawStocks: Stock[],
  ...
  const placementsByStock = new Map<string, ProfilePlacement[]>();
  for (const s of sortedStocks) {
    placementsByStock.set(s.id, []);
  }

  const placedProfiles: ProfilePlacement[] = [];
  ...
  for (const stock of sortedStocks) {
    ...
    if (placements.length > 0) {
      ...
      const placedOnThisStock: PlacedPart[] = placements.map((p) => ({
  ...
  const cutGuidance = generateCutGuidance(
    placedProfiles,
    usedStocks.length > 0 ? { width: usedStocks[0].width, height: usedStocks[0].height } : { width: 1000, height: 1000 }
  );

  const placedParts: PlacedPart[] = placedProfiles.map((p) => ({
    instanceId: p.instanceId,
    groupId: p.groupId,
    name: p.name,
    stockId: p.stockId,
 ...
 * 获取旋转策略允许的角度
 */
function getCandidateAngles(policy?: string): number[] {
    ...
    for (let y = 0; y <= stock.height - bbox.height; y += step) {
      for (let x = 0; x <= stock.width - bbox.width; x += step) {
        ...
        for (const prev of existing) {
          ...
          const overlapBBox = !(
            candBBox.x + candBBox.width + kerf <= pBox.x ||
 ...
 * 候选方案优劣比较器
 */
export function compareProfileCandidates(a: Candidate, b: Candidate): number {
  // 1. 完整放置优先

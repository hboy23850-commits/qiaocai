...
import type { PartGroup, SolverOutput, Stock } from '../types/index.js';

export interface AgentSolveResult { solverOutput: SolverOutput; candidates: AgentCandidateSummary[]; stocks: Stock[]; }
const GOALS = new Set(['BALANCED', 'SAVE_MATERIAL', 'EASY_CUT']);

export function validateAgentTurnRequest(input: Partial<AgentTurnRequest>): { valid: boolean; error?: string } {
  const message = typeof input.message === 'string' ? input.message.trim() : '';
  if (!message) return { valid: false, error: '请输入制作需求' };
  if (message.length > 1000) return { valid: false, error: '单次输入不能超过1000字' };
  return { valid: true };
}
function toScaled(value: number, unit: string): number {
  const mm = unit.toLowerCase() === 'cm' ? value * 10 : value;
  return Math.round(mm * 10);
}
export function createRuleFallbackDraft(text: string): AgentDraft {
  ...
  while ((match = pattern.exec(normalized)) !== null && partGroups.length < 10) {
    ...
    partGroups.push({
      ...
      ...(maxShrinkMm === 1 || maxShrinkMm === 2 ? { flexibleRange: { maxShrinkMm: maxShrinkMm as 1 | 2, stepMm: 1 as const } } : {}),
    });
    index++;
  }
...
export function sanitizeAgentDraft(raw: any, accessibleStocks: Stock[]): AgentDraft {
  ...
  const partGroups: PartGroup[] = rawGroups.map((group: any, index: number) => {
    ...
    return {
      ...
      ...(maxShrink === 1 || maxShrink === 2 ? { flexibleRange: { maxShrinkMm: maxShrink as 1 | 2, stepMm: 1 as const } } : {}),
    };
  });
  return {
    stockIds: Array.isArray(raw?.stockIds) ? [...new Set(raw.stockIds.map(String) as string[])].filter((id) => availableIds.has(id)) : [],
...
export function validateRequirementDraft(draft: AgentDraft, accessibleStocks: Stock[]): AgentDraftValidation {
  ...
  for (const group of draft.partGroups) {
    ...
    if (group.targetWidth > 20000 || group.targetHeight > 20000) errors.push(`${group.name}的尺寸超过2000mm上限`);
...
export function solveAndCompare(draft: AgentDraft, accessibleStocks: Stock[]): AgentSolveResult {
...
export function buildCutSummary(output: SolverOutput) {
